...
static struct netbsd_handle tcp_curr_client[CURRENT_CLIENTS];

static int cli_idx = 0;
static int cc_cli_count = 1;
...
static void timer_10ms_cb(EV_P_ ev_timer *w, int revents) { user_hardclock(); }

void cc_client_connect() {
    static int cur_cc_idx = 0;
    struct netbsd_handle *nh = &tcp_curr_client[cur_cc_idx];
    nh->read_cb = tcp_client_read_cb;
