...
func generateFiles(template string, count int, outputDir string, numaNode int) {
    ...
    switch template {
    case "http_client":
        // Need 1 core for lb_c + count cores for http_client_[id]
        need := 1 + count
        if len(usable) < need {
        ...
        lbCore := usable[0]
        clientCores := usable[1 : 1+count]
        generateHttpClientWithCores(count, outputDir, lbCore, clientCores)

    case "http_server":
        // Need 1 core for lb_s + count cores for http_server_[id]
        need := 1 + count
        if len(usable) < need {
    ...
    case "both":
        ...
        lbCCore := usable[0]
        clientCores := usable[1 : 1+count]
        lbSCore := usable[1+count]
        serverCores := usable[2+count : 2+2*count]
